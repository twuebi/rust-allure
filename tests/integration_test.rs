...
mod test {
    ...
    async fn test() {
        ...
        let (reporter, mut tx, result_rx) =
            Reporter::new("Test it".into(), "Test that".into(), "1");
        let task_handle = tokio::task::spawn(reporter.task());

        let client = ClientBuilder::new(reqwest_client)
            .with(
                LoggingMiddleware::new(PathBuf::from("allure-results"), tx.clone())
                    .await,
            )
            .build();
    ...
    async fn test2() {
        ...
        let reqwest_client = Client::builder().build().unwrap();
        let (reporter, mut tx, result_rx) =
            Reporter::new("Test it2".into(), "Test that2".into(), "1");
        let task_handle = tokio::task::spawn(reporter.task());
    ...
    async fn test3() {
        ...
        let reqwest_client = Client::builder().build().unwrap();
        let (reporter, mut tx, result_rx) =
            Reporter::new("Test it3".into(), "Test that3".into(), "2");
